...
function buildSyncSuggestions(
  ...
  const endDateLabel = chronoResult.endDateLabel;

  if (dateLabel && !dueDate) {
    const label = endDateLabel && endDateLabel !== dateLabel ? `Due: ${dateLabel} - ${endDateLabel}` : `Due: ${dateLabel}`;
    suggestions.push({
      key: 'dt',
      icon: 'calendar-month',
      label: label,
      apply: () => {
        setDate(dateLabel);
        if (endDateLabel && endDateLabel !== dateLabel) {
          setEndDate(endDateLabel);
        }
      },
    });
  }
...
export const TaskComposer = ({
  ...
  useEffect(() => {
    ...
    const syncFound = buildSyncSuggestions(
      title,
      dueDate, dueEndDate, dueTime, priority, reminder, selectedTags, location,
      setDueDate, setDueEndDate, setDueTime, setReminder, setLocation,
  ...
  const resetForm = () => {
    setTitle('');
    setDesc('');
    setPriority(manage.defaultPriority);
    setDueDate('');
    setDueEndDate(null);
    setDueTime(''); setReminder(''); setLocation('');
    setSelectedTags([]); setSubtasks([]); setDesc(''); setAttachments([]);
    setActivePanel(null); setSuggestions([]); setErrors({}); setTouched({});
    setIsSubmitting(false); setSubInput('');
    setShowDatePicker(false);
